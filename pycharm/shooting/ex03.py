...
def runGame(): # 게임 실행함수
    global gamepad, clock, fighter,enemy #전역변수
    ...
    while not doneFlag:
        ...
        #적 비행기 이미지 출력
        enemy_y += enemy_speed #y축 아래로 이동
        if enemy_y > pad_height: #y 좌표
            enemy_y=0 #좌표를 0으로 설정
            enemy_x=random.randrange(0,pad_width-enemy_width)

        drawObject(enemy,enemy_x,enemy_y) #이미지 출력력
